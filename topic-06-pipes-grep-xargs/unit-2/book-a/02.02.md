...
## Example 5 - Making a cow talk with random fortunes

An extra pipe command to try for yourself which uses the previous cowsay package we used combined with a new package called fortune (fortune outputs a frandom quote/joke). Install fortune as shown in command below:

```bash
sudo dnf install fortune-mod
```

The following command will execute `fortune` and pipe `|` the output to `cowsay`.
...
- `fortune` is a program that outputs a random quote or joke.
- `|` is the pipe operator that sends the output of the previous command to the next command.
- `cowsay` receives the text generated and formats it into the speech bubble (shown in picture below).

![Pipe fortune to cowsay](img/cowsaypipe.png)
